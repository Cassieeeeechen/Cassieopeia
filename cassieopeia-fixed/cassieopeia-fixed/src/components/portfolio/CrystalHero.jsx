...
function UniversePortal({ visible, onClose, onDownloadCV, onDownloadCVChinese }) {
      ...
      {visible && (
        <motion.div
          ...
          <motion.div
            ...
            <p className="text-white text-4xl font-extralight tracking-tight opacity-25">{language === 'zh' ? 'Cassie 的空间' : "Cassie's space"}</p>
          </motion.div>
...
export default function CrystalHero() {
  ...
  const handleDownloadCVChinese = () => {
    const cvUrl = '/documents/Cassie_Resume_CN.pdf';
    const link = document.createElement('a');
    link.href = cvUrl;
    link.download = 'Cassie_简历_中文版.pdf';
    link.click();
    setShowCVPopover(false);
  };
  ...
  return (
    <>
      <UniversePortal visible={portalOpen} onClose={handlePortalClose} onDownloadCV={handleDownloadCV} onDownloadCVChinese={handleDownloadCVChinese} />
    ...
    <section
          ...
          {showPopover && (
            <motion.div
              ...
              <span className="text-[12px] font-medium tracking-[0.2em] uppercase" style={{ color: 'rgba(120,100,200,0.8)' }}>
                {language === 'zh' ? '开启入口' : 'ACTIVATE THE PORTAL'}
              </span>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <motion.div
        ...
        <p className="text-[11px] font-medium tracking-[0.45em] uppercase mb-3" style={{ color: 'rgba(130,100,220,0.55)' }}>{language === 'zh' ? '平面设计师' : 'GRAPHIC DESIGNER'}</p>
        <h1 className="text-4xl md:text-6xl font-extralight tracking-tight" style={{ color: '#1a1535' }}>
          {language === 'zh' ? (
            <>雕琢<span className="iridescent-text font-light">视觉诗意</span></>
          ) : (
